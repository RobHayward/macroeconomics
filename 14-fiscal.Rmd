# Fiscal policy
Though monetary policy is the preferred macroeconomic tool under the standard policy framework, fiscal polish is important because it can operate when monetary police is less effective (when at the lower bound for example), it can act as an automatic stabiliser. It has an influence on government deficit and the government debt. Automatic stabilisers and discretionary policy are the two ways that fiscal action can affect the economy. 

Fiscal policy is also important in the microeconomic sphere. It can be used as a way to redistribute income, it can be used for resource allocation and it can be used to provide public goods. There are substantial differences in the level of redistribution that is carried out under the tax and benefit system.  The government may intervene in cases where there are market failure or to implement political choices. The government may also provide public goods. 

### Short run policies
Discretionary fiscal policy can be divided into different time frames. These can be short-run, medium-run and long-run.  This chapter is about the business cycle and the short run.  Medium-term and long-term polices are usually called **Supply-side** policies. These are discussed in the next chapter.  One major area of debate for short-term stabilising policy is the power of the multiplier.  There has been a major debate over the impact of the multiplier since the GFC.  Recall the multiplier is
...
The discussion of fiscal stimulus and the austerity policies have brought the power of the multiplier back into the spotlight. Estimation is complicated by the presence of endogeneity and feedback effects.  Different econometric techniques have been used to try to deal with this problem. Some studies have attempted to find narrative of tax changes to assess the economic impact. See for example, [@RomerRomer]. [@Ramey2011] looks at 30 studies for US multipliers and finds effects of between 0.8% and 1.5%. It is very likely to be the case that the power of the multiplier depends on context and institutions. It is also likely that the multiplier changes in intensity over the course of the economic cycle. The IMF review of 28 economies found a multiplier of between 0.9 and 1.7 for countries in recession. This compares to the 0.5 that is used by the fund in its economic forecasts [@IMF2012].
...
There is some attention on **Modern Monetary Theory** at present. This idea suggests that a government with monetary authority only needs to worry about the inflationary consequences of the budget deficit and debt.  
