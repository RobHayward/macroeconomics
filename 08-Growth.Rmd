...
The capitalist explosion of economic growth is a new phenomena. The upswing began around 1750 in the UK. A natural experiment happened with economic planning in the Soviet Union and other countries.  Here there is no incentive to innovate or create new products. The switch from planning to capitalism was painful, showing institutions and history is important. Planning was very useful for initial development but not for the later stages. There are three broad explanation for what causes different regions to develop at different rates: geography, culture and institutions.  
...
The Solow-Swan model was an extension of the Harrod-Domar model.  It predicts that economies converge to their steady state in the long run and that permanent growth is only achieved by technological progress. Convergence is implied.  Assuming no growth in labour or technological progress, eventually the last capital will only be sufficient to cover depreciation.  At that point the economy will cease to grow. If the labour force grows, the economy converges to a steady start of growth where GDP per capital is constant. If there is technological progress, the per-capital growth will be equal to the rate of technological progress. 

Effective labour AL with A as labour-augmenting technology or knowledge. 

if labour and technology grow at n and g respectively, 

$$L_t = L(0)e^{nt}$$
$$A_t = A(0)e^{gt}$$

Effective labour $A_t$ $L_t$ grows at $n + g$. 


$$\dot{K_t} sY_t - \delta K_t$$

the growth of capital over time is equal to output that is not consumed or worn out. 

k is the capital stock per unit of effective labour. 

$$\dot{k_t} = sk_t^\alpha = (n + g + \delta)k_t$$

$sk_t^\alpha = sy_t$ is the investment per unit of effective labour; $(n + g + \delta)_t$ is the break-even investment: the amount of investment that must be made to prevent k falling. This implies that $k_t$ convergence to $k^*$ where 

$$sk_t^\alpha = (n + g + \delta)k_t $$

means that there is no increase or decrease in capital intensity. 
...
The production function for a one-good economy (corn or something else that can be consumed or used for investment) with 
...
Figure 8.8 shows the steady state where the growth rates of capital, labour and output are equal. This can tell us about the growth of output whereas the more standard Solow diagram in 8.9 shows the growth of output per capita. Page 278 also has the derivation of the Fundamental Solow equation of motion that shows how capital per worker varies over time. 
